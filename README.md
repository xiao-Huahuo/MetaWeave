...
裸文档到 Markdown/frontmatter 采用双链路解析：用户在“OCR/VLM 设置”中配置 MinerU 精准 API Key 并开启 VLM 后，知识灌库优先使用 MinerU `vlm`；未开启 VLM 时显式使用本地解析器，联网任务断网时也会回退本地。两条链路统一输出 Markdown、章节、资源和版面块，之后仍由原有切片、向量入库和知识图谱流程消费。MinerU 不可连接且需要 OCR 时会同步准备 PP-StructureV3，本地模型下载或加载失败则任务明确失败，不生成假空结果。结构化 Markdown 进入 `.mw/md/`，版面块、坐标、页码和类型统计进入 `.mw/frontmatter/`。Agent 会话附件保持既有本地解析与本地 Qwen 视觉补充，不随知识库 VLM 总开关自动外传。
