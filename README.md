...
图片、扫描型 PDF 以及文档内嵌图片可以使用 PaddleOCR 提取中英文文字。OCR 默认关闭，用户在设置页开启后会对后续灌库立即生效，无需重启应用。用户直接上传到 Agent 会话的图片会先完成 OCR，再交给 CPU 本地 Qwen 补充对象、布局、空间关系、图表趋势和其他视觉语义；两部分合并为附件正文。Agent 也可以调用“识图”工具，对当前会话中的指定图片提出新的视觉问题。
