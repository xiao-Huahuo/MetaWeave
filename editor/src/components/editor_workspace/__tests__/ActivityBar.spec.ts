...
describe('ActivityBar', () => {
  ...
  it('uses the queue-lane frame with mode-specific outer corners', () => {
    ...
    expect(editorWorkspaceSource).toMatch(
      /\.editor-sidebar-content,[\s\S]*?\.agent-col \{[^}]*border: 1px solid var\(--workspace-panel-border\);[^}]*outline: 2px solid var\(--workspace-panel-outline\);[^}]*outline-offset: 2px;[^}]*box-shadow: 0 0 0 2px var\(--workspace-panel-ring\);/s,
    )
    expect(editorWorkspaceSource).toMatch(
