...
| 内存 | 基础功能至少 8 GB；使用 Embedding、ReRank 或 OCR 时建议 16 GB 或以上 |
...
* 模型接入：以 `deepseek-flash` 作为配置参考，支持用户配置 OpenAI-compatible 大模型、小模型和视觉模型接口；未配置有效远程大模型时返回明确设置提示，不再回退本地语言模型。
...
* 文档解析：MinerU 精准 API（联网优先）+ PP-StructureV3（显式本地/断网回退）
...
裸文档到 Markdown/frontmatter 采用双链路解析：用户在“OCR/VLM 设置”中配置 MinerU 精准 API Key 并开启 VLM 后，知识灌库和 Agent 的按需文件读取优先使用 MinerU `vlm`；未开启 VLM 时显式使用本地解析器，联网任务断网时也会回退本地。两条链路统一输出 Markdown、章节、资源和版面块，之后仍由原有切片、向量入库和知识图谱流程消费。MinerU 不可连接且需要 OCR 时会同步准备 PP-StructureV3，本地模型下载或加载失败则任务明确失败，不生成假空结果。结构化 Markdown 进入 `.mw/md/`，版面块、坐标、页码和类型统计进入 `.mw/frontmatter/`。Agent 会话附件上传只保存原文件；Agent 调用 `read_file` 时才按需解析并缓存，调用 `understand_image` 时由远程视觉模型直接读取原图。
...
#### 模型说明
##### 模型选型
项目不再内置本地对话或视觉模型。Agent 需要配置可用的 OpenAI-compatible 远程大模型；视觉模型可以独立配置，也可以留空继承大模型。

注：可采用 `deepseek-flash` 作为大模型、小模型或视觉模型配置参考；实际模型必须与所用接口能力匹配。
##### 大模型API设置
...
**模型任务调度器**统一管理所有 LLM 调用。内部多级队列按主 Agent、Summary、Fact Extraction 三个等级分配,同时设置 `large / small` 双模型池路由——主推理走大模型池,摘要/事实抽取/上下文压缩走小模型池,分别配备独立并发上限、超时、熔断与重试机制。
    ...
    * 物理模型隔离：
      * 用户没有配置可用远程大模型时，Agent 返回明确配置提示，不执行主模型或小模型任务；独立视觉端点仍可用于已授权的图片理解；
      * 用户配置了大模型但没有配置小模型时，小模型任务复用大模型的模型名、Base URL 和 API Key，但仍占用小模型池的并发配额;
      * 大小模型都配置时分别调用各自配置；视觉模型可独立配置，也可完整继承大模型，供上传图片的视觉理解与“识图”工具复用。
...
扫描器用于把临时文件或网页资料转换为可编辑的 Markdown。上传区设置菜单提供任务级“OCR”和“联网”：联网默认跟随用户是否已开启 VLM，但只在当前页面临时生效，不回写持久设置；每个任务提交时固化选择。联网任务显示 MinerU 的真实异步阶段，不伪造百分比；显式本地或断网回退后继续显示本地流水线的真实进度。完成结果保存在当前知识库的 `.mw/scan/<scan_id>/`，历史记录、解析器来源、回退原因、收藏、草稿和导出状态持久化到 SQLite。
...
扫描队列复用扫描器的持久化异步任务机制和同一个解析设置菜单。队列顶栏的临时选择应用于之后新建的全部任务，已排队任务不受后续修改影响。看板以“等待扫描 / 扫描中 / 扫描完成”集中处理多个文件和网页；完成列只显示当天结果，更早的完成记录进入历史页。

##### OCR

扫描器与知识灌库共享同一个“裸文档 → Markdown/frontmatter”入口。联网时使用 MinerU 精准 API，默认读取其 Markdown，并把 `content_list.json` 的 0–1000 归一化 bbox 映射到原件页面尺寸后复用现有矩形框交互；本地时使用 **PP-StructureV3 高质量流水线**。Agent 图片附件不进入这条联网双链路。
...
整套本地模型作为一个 `paddleocr` 受管模型组保存在 `runtime/models/paddleocr/`。应用启动和 MinerU 正常路径不会自动加载或下载它；只有用户显式选择本地 OCR，或联网失败需要本地回退时，才同步下载并加载完整流水线。准备失败时 OCR 不会开启，当前任务明确失败。旧的 Mobile 检测/识别完成标记不再视为完整。
...
模型管理覆盖 Embedding、ReRank 和 PaddleOCR 结构化流水线，存储路径树同步展示这些本地检索与 OCR 模型的真实占用。每个模型首行显示配置名称、磁盘大小、下载/加载状态、当前是否启用和操作入口；展开 PaddleOCR 项可以核对版面、Server OCR、表格、公式、文档预处理模型以及明确关闭的模块。Hugging Face 模型按仓库总字节和实际落盘字节显示真实百分比；PaddleOCR 无法提前获得整组总量时只显示实时落盘字节和当前阶段，不生成假百分比。
